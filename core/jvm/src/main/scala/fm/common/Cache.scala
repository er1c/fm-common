...
import com.google.common.cache.{
  Cache => GoogleCache,
  CacheBuilder => GoogleCacheBuilder,
  CacheLoader => GoogleCacheLoader,
  CacheStats => GoogleCacheStats,
  LoadingCache => GoogleLoadingCache,
  RemovalCause => GoogleRemovalCause,
  RemovalListener => GoogleRemovalListener,
  RemovalNotification => GoogleRemovalNotification
}
...
object Cache {
  ...
  private class GoogleRemovalListenerAdapter[K, V](removalListener: RemovalNotification[K, V] => Unit)
    extends GoogleRemovalListener[K, V] {
...
sealed class Cache[K, V](cache: GoogleCache[K, V]) {
  ...
  def putAll(entries: Iterable[(K, V)]): Unit = entries.foreach { case (k, v) => put(k, v) }
...
object LoadingCache {
  ...
  final class InvalidCacheLoadException(msg: String, cause: Throwable, stackTrace: Array[StackTraceElement])
  ...
  object CacheLoader {
    def apply[K, V](loader: K => V): CacheLoader[K, V] =
      new CacheLoader[K, V] {
     ...
     */
    def apply[K, V](loader: (K, Option[V]) => V): CacheLoader[K, V] =
      new CacheLoader[K, V] {
  ...
  abstract class CacheLoader[K, V] {
    ...
    def loadAll(keys: Iterable[K]): Map[K, V] =
      ??? // This is intentional to trigger the default GoogleCacheLoader behavior if this method is not overridden
    def reload(key: K, oldValue: V): Future[V] = { void(oldValue); Future.successful(load(key)) }
  }
  ...
  private class GoogleCacheLoaderAdapter[K, V](loader: CacheLoader[K, V]) extends GoogleCacheLoader[K, V] {
    ...
    override def loadAll(keys: JavaIterable[_ <: K]): JavaMap[K, V] =
      try { loader.loadAll(keys.asScala).asJava }
      catch { case _: NotImplementedError => super.loadAll(keys) }
    override def reload(key: K, oldValue: V): ListenableFuture[V] =
      new GoogleListenableFutureAdapter[V](loader.reload(key, oldValue))
  }
  ...
  private val exceptionHandler: PartialFunction[Throwable, Nothing] = {
    case ex: GoogleCacheLoader.InvalidCacheLoadException =>
      throw new InvalidCacheLoadException(ex.getMessage, ex.getCause, ex.getStackTrace)
...
final class LoadingCache[K, V](cache: GoogleLoadingCache[K, V]) extends Cache(cache) {
  ...
  def get(key: K): V =
    try { cache.get(key) }
    catch LoadingCache.exceptionHandler
