...
object IP {
  ...
  def isValid(ip: String): Boolean =
    try { toInt(ip); true }
    catch { case _: InvalidIPException => false }
  ...
  def findAllIPsIn(ips: String): IndexedSeq[IP] =
    if (null == ips) Vector.empty
    else ipv4Pattern.findAllIn(ips).matchData.map { _.group(2) }.map { apply }.toIndexedSeq
  ...
  def get(ip: String): Option[IP] =
    try { Some(apply(ip)) }
    catch { case _: InvalidIPException => None }
  ...
  def apply(ip: String): IP = {
    ...
    try {
      ...
      case _: NumberFormatException =>
        try {
          apply(InetAddress.getByName(ip))
        } catch {
          case _: NoSuchElementException => throw new InvalidIPException("Not sure how to parse: " + ip)
        }
    }
  }
  ...
  def toInt(ip: String): Int = {
    ...
    val octets = fixedIp.trim.split('.').map {
      _.toShortOptionCached.filter { n: Short => n >= 0 && n <= 255 }.getOrElse {
        throw InvalidIPException("Invalid IP Address: " + fixedIp)
      }.toByte
    }
