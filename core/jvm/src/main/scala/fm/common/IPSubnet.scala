...
object IPSubnet {
  ...
  def forRangeOrMask(from: IP, toOrMask: IP): IPSubnet = {
    ...
    require(
      !(validMask && validRange),
      "toOrMask parameter is ambiguous since it looks like both a valid mask and valid range")
...
final case class IPSubnet(ip: IP, bits: Int) extends IPOrSubnet {
  ...
  require(
    ip.intValue >>> shift << shift == ip.intValue,
    s"Subnet IP has non-zero bits when they should be zero.  IP: $ip  ${Integer
      .toBinaryString(ip.intValue)
      .lPad(32, '0')}  Mask: ${Integer.toBinaryString(mask).lPad(32, '0')}"
  )
