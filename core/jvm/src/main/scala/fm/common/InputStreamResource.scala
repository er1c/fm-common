...
object InputStreamResource {
  ...
  def wrap(
    ...
    autoBuffer: Boolean = true): InputStreamResource = {
    InputStreamResource(
      SingleUseResource(is),
      fileName = fileName,
      autoDecompress = autoDecompress,
  ...
  def forFileOrResource(
    ...
    val resource: Resource[InputStream] = MultiUseResource {
      if (file.isFile && file.canRead) newFileInputStream(file, autoDecompress)
      else classLoader.getResourceAsStream(file.toResourcePath)
    }.map { is: InputStream =>
  ...
  def forFile(
    ...
    val resource: Resource[InputStream] = MultiUseResource { newFileInputStream(file, autoDecompress) }.map {
      is: InputStream =>
  ...
  def forResource(
    ...
    val resource: Resource[InputStream] = MultiUseResource {
      classLoader.getResourceAsStream(file.toResourcePath)
    }.map { is: InputStream =>
...
final case class InputStreamResource(
  ...
  import InputStreamResource.{newBOMInputStream, newBOMInputStreamReader, BOMCharsets}
  ...
  private def nullProtectedResource: Resource[InputStream] =
    resource.map { is: InputStream =>
      if (null == is) throw new IOException("InputStream is null!" + fileName.toBlankOption.map { n: String =>
        s"  fileName: $n"
      })
      is
    }
  ...
  def detectCharsetName(): Option[String] =
    use { is: InputStream => IOUtils.detectCharsetName(is, useMarkReset = false) }
  ...
  def bufferedReader(encoding: String): Resource[BufferedReader] =
    reader(encoding) flatMap { r => Resource(new BufferedReader(r)) }
  def bufferedReader(cs: Charset): Resource[BufferedReader] =
    reader(cs) flatMap { r => Resource(new BufferedReader(r)) }
