...
import fm.common.TaskRunnerBase.{
  ClearingBlockRunnableWithPriority,
...
object PriorityTaskRunner extends Logging {
  ...
  private object RunnableWithPriorityComparator extends Comparator[Runnable] {
    override def compare(a: Runnable, b: Runnable): Int = {
      java.lang.Long
        .compare(a.asInstanceOf[RunnableWithPriority].priority, b.asInstanceOf[RunnableWithPriority].priority)
    }
  }

  private class BoundedPriorityQueue[E](queueSize: Int, comparator: Comparator[E])
    extends PriorityBlockingQueue[E](queueSize, comparator) {
 ...
 */
final class PriorityTaskRunner(val name: String, val coreThreads: Int, val maxThreads: Int, val queueSize: Int)
  extends TaskRunnerBase(name) {
  ...
  protected val executor: ThreadPoolExecutor = {
    ...
    val exec: ThreadPoolExecutor = new ThreadPoolExecutor(
      coreThreads,
      maxThreads,
      60,
      TimeUnit.SECONDS,
      queue,
      TaskRunnerBase.newTaskRunnerThreadFactory(name),
      rejectedHandler)
