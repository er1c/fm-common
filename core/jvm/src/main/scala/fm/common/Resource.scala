...
object Resource {
  ...
  private def closeReflectiveCloseable[R <: ReflectiveCloseable](resource: ReflectiveCloseable): Unit = {
    if (resource.isNull) return

    resource match {
      case auto: AutoCloseable => auto.close()
      case other: ReflectiveCloseable @unchecked =>
  ...
  def using[T, R <: ReflectiveCloseable](resource: R)(f: R => T): T =
    try {
      ...
      closeReflectiveCloseable(resource)
    }
  ...
  def using[T, R <: ReflectiveCloseable](resources: Seq[R])(f: Seq[R] => T): T =
    try {
      ...
      if (null != resources) resources.foreach { closeReflectiveCloseable }
    }

  def apply[T <: ReflectiveCloseable](resource: T): Resource[T] = toResource(resource)

  implicit def toResource[T <: ReflectiveCloseable](resource: T): Resource[T] = SingleUseResource(resource)
  ...
  //    val tpeParams: String = letters.map{ _.toUpper+" <: ReflectiveCloseable" }.mkString(", ")
  ...
  def using[RES, A <: ReflectiveCloseable, B <: ReflectiveCloseable](a: A, b: B)(fun: (A, B) => RES): RES = use(SingleUseResource(a), SingleUseResource(b))(fun)
  def using[RES, A <: ReflectiveCloseable, B <: ReflectiveCloseable, C <: ReflectiveCloseable](a: A, b: B, c: C)(fun: (A, B, C) => RES): RES = use(SingleUseResource(a), SingleUseResource(b), SingleUseResource(c))(fun)
  def using[RES, A <: ReflectiveCloseable, B <: ReflectiveCloseable, C <: ReflectiveCloseable, D <: ReflectiveCloseable](a: A, b: B, c: C, d: D)(fun: (A, B, C, D) => RES): RES = use(SingleUseResource(a), SingleUseResource(b), SingleUseResource(c), SingleUseResource(d))(fun)
  def using[RES, A <: ReflectiveCloseable, B <: ReflectiveCloseable, C <: ReflectiveCloseable, D <: ReflectiveCloseable, E <: ReflectiveCloseable](a: A, b: B, c: C, d: D, e: E)(fun: (A, B, C, D, E) => RES): RES = use(SingleUseResource(a), SingleUseResource(b), SingleUseResource(c), SingleUseResource(d), SingleUseResource(e))(fun)
  def using[RES, A <: ReflectiveCloseable, B <: ReflectiveCloseable, C <: ReflectiveCloseable, D <: ReflectiveCloseable, E <: ReflectiveCloseable, F <: ReflectiveCloseable](a: A, b: B, c: C, d: D, e: E, f: F)(fun: (A, B, C, D, E, F) => RES): RES = use(SingleUseResource(a), SingleUseResource(b), SingleUseResource(c), SingleUseResource(d), SingleUseResource(e), SingleUseResource(f))(fun)
  def using[RES, A <: ReflectiveCloseable, B <: ReflectiveCloseable, C <: ReflectiveCloseable, D <: ReflectiveCloseable, E <: ReflectiveCloseable, F <: ReflectiveCloseable, G <: ReflectiveCloseable](a: A, b: B, c: C, d: D, e: E, f: F, g: G)(fun: (A, B, C, D, E, F, G) => RES): RES = use(SingleUseResource(a), SingleUseResource(b), SingleUseResource(c), SingleUseResource(d), SingleUseResource(e), SingleUseResource(f), SingleUseResource(g))(fun)
...
object MultiUseResource {
  def apply[A <: ReflectiveCloseable](makeResource: => A) = new MultiUseResource(makeResource)
}

/**
 * A Resource that can be used multiple times (e.g. opening an InputStream or Reader for a File)
 */
final class MultiUseResource[+A <: ReflectiveCloseable](makeResource: => A) extends Resource[A] {
...
object SingleUseResource {
  def apply[A <: ReflectiveCloseable](resource: A) = new SingleUseResource(resource)
}

/**
 * A Resource that can only be used once (e.g. reading an InputStream)
 */
final class SingleUseResource[+A <: ReflectiveCloseable](resource: A) extends Resource[A] {
