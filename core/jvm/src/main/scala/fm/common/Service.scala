...
object Service {
  ...
  def call[X](
    ...
    delayBetweenCalls: FiniteDuration =
      Service.defaultDelayBetweenCalls, // This is a constant delay before making the call (e.g. for rate limiting to external services)
  ...
  def callAsync[X](
    ...
    timer: ScheduledTaskRunner = ScheduledTaskRunner.global): Future[X] = {
    callAsync0(
      msg,
      logging,
      exceptionHandler,
  ...
  private def callAsync0[X](
    ...
    if (tryCount >= maxRetries)
      return Future.failed(new Exception(s"Service Failed after $maxRetries retries", lastException))
