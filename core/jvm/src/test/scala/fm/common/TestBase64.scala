...
final class TestBase64 extends AnyFunSuite with Matchers {
  ...
  private def check(original: String, encoded: String): Unit = {
    ...
    if (Vector('+', '-', '/', '_').exists { c: Char => encoded.indexOf(c.toInt) != -1 }) {
  ...
  private def makeURLToStrictInputStream(in: String): InputStream =
    new Base64.URLToStrictInputStream(new ByteArrayInputStream(in.getBytes(UTF_8)))
