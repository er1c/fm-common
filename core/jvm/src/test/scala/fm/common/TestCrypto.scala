...
final class TestCrypto extends AnyFunSuite with Matchers {
  ...
  private def encrypt(
    ...
    if (encryptedBase64URLSafeDefault.isDefined)
      defaultCrypto.decryptBase64String(encryptedBase64URLSafeDefault.get) shouldBe msg

    if (encryptedBase64Authenticated.isDefined)
      authenticatedCrypto.decryptBase64String(encryptedBase64Authenticated.get) shouldBe msg
    if (encryptedBase64URLSafeAuthenticated.isDefined)
      authenticatedCrypto.decryptBase64String(encryptedBase64URLSafeAuthenticated.get) shouldBe msg
