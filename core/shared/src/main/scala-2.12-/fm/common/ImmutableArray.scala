...
object ImmutableArray {
  ...
  implicit def canBuildFrom[A]: CanBuildFrom[Coll, A, ImmutableArray[A]] =
  ...
  private class CBF[Elem](makeBuilder: => ImmutableArrayBuilder[Elem])
    extends CanBuildFrom[Coll, Elem, ImmutableArray[Elem]] {
...
final class ImmutableArray[@specialized +A: ClassTag](arr: Array[A])
  extends IndexedSeq[A] with IndexedSeqOptimized[A, ImmutableArray[A]] {
...
final class ImmutableArrayBuilder[@specialized A: ClassTag](initialSize: Int) extends Builder[A, ImmutableArray[A]] {
  ...
  override def ++=(io: IterableOnce[A]): this.type = {
    if (io.hasDefiniteSize) ensureCapacity(_length + io.size)
