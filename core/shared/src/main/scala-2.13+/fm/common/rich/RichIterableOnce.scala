...
final class RichIterableOnce[A](val self: IterableOnce[A]) extends AnyVal {
  ...
  def toUniqueHashMap[K, V](implicit ev: A <:< (K, V)): immutable.HashMap[K, V] = {
    ...
    for (x <- self.iterator) {
      ...
      require(
        !m.contains(key),
        s"RichIterableOnce.toUniqueHashMap - Map already contains key: $key   Existing Value: ${m(
...
//  }
  ...
  def toUniqueHashMapWithTransforms[K, V, K2, V2](keyTransform: K => K2, valueTransform: V => V2)(implicit
    ...
    for (x <- self.iterator) {
      ...
      require(
        !m.contains(key),
        s"RichIterableOnce.toUniqueHashMap - Map already contains key: $key   Existing Value: ${m(key)}  Trying to add value: $value")
      m += ((key, value))
    }
  ...
  def toMultiValuedMapUsingKeys[K](toKeys: A => IterableOnce[K]): immutable.HashMap[K, IndexedSeq[A]] = {
    ...
    for (value <- self.iterator; key <- toKeys(value).iterator) {
      val values: Vector[A] = m.get(key) match {
        case Some(existing) => existing :+ value
        case None => Vector(value)
      }
