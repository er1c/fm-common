...
import scala.collection.{mutable, BuildFrom, IterableOps, MapOps}
import scala.collection.immutable.SortedMap

final class RichMap[A, B, CC[_, _] <: IterableOps[_, Any, _], This <: Map[A, B]](val self: MapOps[A, B, CC, This])
  extends AnyVal {
  /**
  ...
  @inline def mapValuesStrict[C, That <: Map[A, C]](f: B => C)(implicit bf: BuildFrom[This, (A, C), That]): That = {
    val builder: mutable.Builder[(A, C), That] = bf.newBuilder(self.asInstanceOf[This])
