  ...
  def apply(queryString: String): QueryParams = {
    ...
    val params: Seq[(String, String)] = {
      ...
      rawParams.foreach { param: String =>
        ...
        if (idx < 0) {
          b += StringEscapeUtils.decodeURIComponent(param) -> null
        } else {
          b += StringEscapeUtils.decodeURIComponent(param.substring(0, idx)) -> StringEscapeUtils.decodeURIComponent(
...
final class QueryParams private (
  ...
  def updated(key: String, value: String): QueryParams = {
    ...
    val newParams: Seq[(String, String)] = params.flatMap {
      case (k, v) =>
        if (key === k) {
          if (!found) {
            found = true
   ...
   */
  def withoutBlankValues(): QueryParams = filter { case (k @ _, v) => v.isNotNullOrBlank }
   ...
   */
  def remove(keys: String*): QueryParams = filterNot { case (k, v @ _) => keys.contains(k) }
  ...
  private def nonNullValuesForKey(key: String): Seq[String] =
    params.filter { case (k, v) => k === key && v != null }.map { case (k @ _, v) => v }.toList
  private def nonBlankValuesForKey(key: String): Seq[String] =
    params.filter { case (k, v) => k === key && v.isNotNullOrBlank }.map { case (k @ _, v) => v }.toList

  /**
   * The unique set of keys with or without values
   */
  def allKeys: Set[String] = params.map { case (k, v @ _) => k }.toSet

  /**
   * The unique set of keys with non-null values (blank is a valid value)
   */
  def keysWithValues: Set[String] = params.filterNot { case (k @ _, v) => v.isNull }.map { case (k, v @ _) => k }.toSet

  /**
   * The unique set of keys with non-blank values
   */
  def keysWithNonBlankValues: Set[String] =
    params.filterNot { case (k @ _, v) => v.isNullOrBlank }.map { case (k, v @ _) => k }.toSet

  /**
   * The unique set of keys without values
   */
  def keysWithoutValues: Set[String] = params.filter { case (k @ _, v) => null == v }.map { case (k, v @ _) => k }.toSet

  /**
   * Convert to a Map[String, Seq[String]]
   */
  def toMap: Map[String, Seq[String]] =
    params.groupBy { case (k, v @ _) => k }.mapValuesStrict { values: Seq[(String, String)] =>
      values.map { _._2 }.filterNot { _ === null }
    }
  ...
  def withPrefixedKeys(prefix: String): QueryParams = mapKeys { prefix + _ }
