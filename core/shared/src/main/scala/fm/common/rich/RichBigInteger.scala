...
import fm.common.Implicits.{toAnyRefNullChecks, toRichSomeObject}
...
final class RichBigInteger(val self: BigInteger) extends AnyVal with Ordered[BigInteger] {
  ...
  def intValueExact(): Int = {
    if (lt(self, IntMin) || gt(self, IntMax))
      throw new ArithmeticException("BigInteger is outside the range of an Int: " + self)
    self.intValue()
  }
  ...
  def longValueExact(): Long = {
    if (lt(self, LongMin) || gt(self, LongMax))
      throw new ArithmeticException("BigInteger is outside the range of an Long: " + self)
    self.longValue()
  }
