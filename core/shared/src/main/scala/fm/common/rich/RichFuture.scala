...
object RichFuture {
  def apply[V](f: Future[V]): RichFuture[V] =
    if (f.isInstanceOf[RichFuture[_]]) f.asInstanceOf[RichFuture[V]] else new RichFuture(f)
   ...
   */
  @inline def exists[T](futuretravonce: scala.collection.immutable.Iterable[Future[T]])(predicate: T => Boolean)(
